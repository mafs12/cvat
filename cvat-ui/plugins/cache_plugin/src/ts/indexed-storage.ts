...
class CVATIndexedStorage {
    ...
    #storeConfiguration = {
        ...
        [DatabaseStore.FUNCTIONS_PREVIEW]: {
            options: { keyPath: 'id' },
            validator: (el: string) => isInstance(el, 'string', DatabaseStore.FUNCTIONS_PREVIEW),
            serialize: (el: string) => el,
        },
        [DatabaseStore.COMPRESSED_JOB_IMAGES]: {
            options: { keyPath: 'id' },
            validator: (el: JobImage) => el.imageData instanceof ImageBitmap &&
                Number.isInteger(el.renderHeight) && Number.isInteger(el.renderWidth),
            serialize: (el: JobImage) => ({
                imageData: el.imageData,
                renderWidth: el.renderWidth,
                renderHeight: el.renderHeight,
            }),
        },
        [DatabaseStore.CONTEXT_IMAGES]: {
            options: { keyPath: 'id' },
            validator: (el: any) => !!el, // todo: check actual type
            serialize: (el: string) => el,
        },
    };
