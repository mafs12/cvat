        ...
        return new Promise<{
            ...
            if (frame) {
                if (decodeForward && decodedBlocksCacheSize > 1 && !frameDataCache[this.jobID].activeChunkRequest) {
                    let firstFrameInNextChunk = this.number + forwardStep;
                    ...
                    // TODO: implement better decode forward
                    if (nextChunkNumber * chunkSize <= stopFrame && !provider.isChunkCached(nextChunkNumber)) {
                        frameDataCache[this.jobID].activeChunkRequest = new Promise((resolveForward) => {
                            serverProxy.frames
                                .getData(this.jobID, nextChunkNumber).then((chunk: ArrayBuffer) => {
            ...
            (frameDataCache[this.jobID].activeChunkRequest || Promise.resolve()).finally(() => {
                ...
                frameDataCache[this.jobID].activeChunkRequest = new Promise<void>((
                    ...
                    serverProxy.frames.getData(this.jobID, chunkNumber).then((chunk: ArrayBuffer) => {
                        try {
                            provider
                                .requestDecodeBlock(
                                    chunk,
