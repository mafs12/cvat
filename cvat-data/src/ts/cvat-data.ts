...
export class FrameDecoder {
    ...
    */
    ...
    async _cleanup(): Promise<void> {
        // removes chunks around the current one exceeding the limit
        const currentChunkNumber = Math.floor(this.currentFrame / this.chunkSize);
        const distance = Math.floor((this.cachedChunksLimit - 1) / 2);
        const chunks = Object.keys(this.decodedChunks).map((chunk: string) => +chunk);
        const prevChunks = chunks.filter((chunk) => chunk < currentChunkNumber);
        const nextChunks = chunks.filter((chunk) => chunk > currentChunkNumber);
        const prevChunksToRemove = prevChunks.slice(0, prevChunks.length - distance);
        const nextChunksToRemove = nextChunks.slice(distance, nextChunks.length);

        [].concat(...prevChunksToRemove, ...nextChunksToRemove).forEach((chunk: number) => {
            // TODO: think how to improve if one of sides does not have decoded chunks
            // probably we need to know direction here
            delete this.decodedChunks[chunk];
        });
    }
            ...
            if (this.blockType === BlockType.MP4VIDEO) {
                ...
                worker.onmessage = (e) => {
                    ...
                    createImageBitmap(new ImageData(array, width)).then((bitmap) => {
                        ...
                        if (keptIndex in this.promisedFrames) {
                            ...
                            resolve(decodedFrames[keptIndex]);
                        }
                ...
                this.zipWorker.onmessage = async (event) => {
                    ...
                    if (event.data.index in this.promisedFrames) {
                        ...
                        resolve(decodedFrames[event.data.index]);
                    }
